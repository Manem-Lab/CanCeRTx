...
<br>9 McMahon, Quebec, QC, Canada G1R 3S3"
  ...
  dist_df <- reactive({
    ...
    isolate({
      ...
      if (input$dist_tissue == "All Tissue Types"){
        ...
        t <- (object@SampleData$gamma[object@SampleData$gamma$tissueid == input$dist_tissue,])[,1]
        t <- intersect(t, object@DoseResponseData$gamma$Cell_line)
  ...
  output$drcdesc <- renderUI({
    HTML(
      paste(
        "Cell Lines: Compare cell lines in one dataset",
        "Datasets: Compare a cell line through multiple datasets",
        sep = "<br/>"
      )
    )
  })
  ...
  output$hypoxia_chart <- renderPlot({
    ...
    isolate({
      ...
      wilcox_test <- wilcox.test(sf160, sfoxy, paired = TRUE)
      p_label <- paste0("Wilcoxon p = ", formatC(wilcox_test$p.value, format = "e", digits = 2))
  ...
  output$gdataset1 <- renderUI({
    selectizeInput(inputId = "gdataset1",
                   label = NULL,
                   choices = c("Search Bar" = "", datalist[-2,]),
                   multiple = FALSE,
                   selected = NULL,
                   options = list(
                     create = FALSE,
  ...
  output$gea_chart <- renderPlotly({
    ...
    print(statistic)
    print(p.value)
    stars <- ifelse(p.value < 0.001, "***", ifelse(p.value < 0.01, "**", ifelse(p.value < 0.05, "*", paste0(p.value, " (ns)"))))
    ...
    p <- plot_ly(
      ...
      boxpoints = "all", jitter = 2.0, pointpos = 0
    ) %>%
      # add_trace(
      #   y = ~Value,
      #   color = ~Source,
  ...
  output$gexport1 <- downloadHandler(
    ...
    content = function(file) {
      ...
      p <- plot_ly(
        ...
        # ) %>%
        layout(
          title = paste0("Gene Expression of ", gene, " Stratified by Dose Response"),
          xaxis = list(
            title = ""
          ),
          yaxis = list(
  ...
  pa_df <- reactive({
    ...
    isolate({
      ...
      if (input$pa_rad == "Gamma"){
        ...
        View(object@GeneExpressionData)
        ...
        metric <- data.frame(rownames(df1), cor.val)
